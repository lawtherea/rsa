...
long long verificar(long long int e, long long int p, long long int q,long long int *pont)
{
	...
	if(mdc(e, auxiliar) != 1)
	{
		printf("O número %lld não é relativamente primo a (p - 1)(q - 1), redigite-o: ", e);
		scanf("%lld", pont);
		e = *pont;
...
long long criararquivocr(long long int chave[], long long int n,long long int e,int cont1)
{
	int cont;
	FILE *chav;
	chav= fopen("criptografado.txt", "w");
...
long long criararquivodecr(char chave[])
{
	...
	fprintf(chav,"O texto descriptografado é: %s", chave);
	fclose(chav);
}
int main()
{
	...
	while(1)
	{
		...
		if(funcao == 1)
		{
			...
			criararquivo(p*q,e);
			printf("\n\nCaso deseje voltar ao menu, digite 1. Caso não, digite 0 para encerrar o programa: ");
		...
		if(funcao == 3)
		{
			...
			FILE *chav;
			chav= fopen("criptografado.txt", "r");
			contador = 0;
